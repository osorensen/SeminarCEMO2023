...
.footnote[
GLLAMM: Rabe-Hesketh, S., Skrondal, A., & Pickles, A. (2004).  Psychometrika, 69(2), 167–190. https://doi.org/10.1007/BF02295939
]
...
- Control of stepsize $\tau$ actually necessary in practice.

---

# Outer loop

- Cholesky factors and design matrices are linearized in the inner loop, but need to be updated between each step of the outer loop. Mappings to sparse matrix representations thus required.
...
# Old style and new style random effects

Terminology introduced by Hodges (2014):

### Old-style random effects

- Effects that are sampled randomly each time we take a new dataset.
...
- Mixed model machinery used to enforce smoothing. Effects not actually sampled.
